...
app.get('/api/invoices/:id/pdf', (req, res) => {
  try {
    ...
    if (invoice.customer_phone) {
      try {
        invoice.customer_phone = decrypt(invoice.customer_phone);
      } catch (e) {
    ...
    if (invoice.customer_email) {
      try {
        invoice.customer_email = decrypt(invoice.customer_email);
      } catch (e) {
        // If decryption fails, data might not be encrypted
      }
    }
    ...
    let job = null;
    if (invoice.job_id) {
      job = db.prepare('SELECT labor_hours, labor_rate, labor_cost, fuel_charge, cleaning_charge FROM jobs WHERE id = ?').get(invoice.job_id);
    }
